...
A self-leveling/bubble level driver for Tasmota using Berry scripting and the MPU6050 IMU sensor. There is no need to install Tasmota's native MPU6050 driver (which requires re-compiling the Tasmota system).
...
**First time only** - do this in the Tasmota web console:

1. **Enable I2C** in Tasmota configuration (Configure Module → I2C SDA/SCL)

2. **Upload `level.be`** to your Tasmota device

3. **Load the driver interactively**:
   ```
   br load('level')
   ```
...
5. **Calibrate**:
   ...
   LEVEL: Calibrated: [0.8727, -0.4452, 0.2007]
   ```
...
6. **Test the readings**:
   ...
   Should show ~0° when level, and increase as you tilt the device.
...
```berry
load('level')  # Automatically loads driver and restores calibration
```
